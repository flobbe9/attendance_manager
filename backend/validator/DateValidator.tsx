...
export class DateValidator extends AbstractAttendanceInputValidator<Date> {
     ...
     */
    public getInvalidValues(): Map<Date, string> {
        const invalidValues = new Map<Date, string>();
        ...
        if (this.getCurrentAttendance().examinants) {
            ...
            this.getCurrentAttendance().examinants.forEach((examinantEntity) => {
                this.attendanceService.findAllByExaminant(savedAttendances, examinantEntity.role).forEach((savedAttendance) => {
                    if (savedAttendance.date)
                        invalidValues.set(
                            savedAttendance.date,
                            `An diesem Tag ist der Prüfer für '${getExamiantRoleByExaminantRoleKey(
                                examinantEntity.role
                            )}' bereits in einem anderen UB eingeplant.`
                        );
                });
            });
        }
    ...
    public formatInvalidValues(invalidValues?: Map<Date, string>): ReactNode {
        if (!invalidValues)
            invalidValues = this.getInvalidValues();

        return Array.from(invalidValues.entries())
                    .map(([date, errorMessage], i) => {
                        const formattedDate = formatDateGermanNoTime(date);
    ...
    public formatValidValues(values?: unknown): ReactNode {
        // not implemented
        return "";
    }
        ...
        try {
            for (const examinantEntity of this.getCurrentAttendance().examinants)
                if (examinantsPresentOnSelectedDate.has(examinantEntity.role))
                    throw new Error(
                        `Ein Prüfer mit der Rolle '${getExamiantRoleByExaminantRoleKey(examinantEntity.role)}' ist am ${formatDateGermanNoTime(
                            selectedDate
