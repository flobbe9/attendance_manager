...
import { HISTORY_SCHOOL_YEAR_CONDITIONS_VARIANT_1, HISTORY_SCHOOL_YEAR_CONDITIONS_VARIANT_2, MUSIC_SCHOOL_YEAR_CONDITIONS, MUSIC_SCHOOL_YEAR_TOPIC_CONDITIONS } from "@/utils/attendanceValidationConstants";
...
export class HistorySchoolYearValidator extends AbstractSchoolYearValidator {
    ...
    public validateContextConditions(allConstantSchoolYearConditions: SchoolYearCondition[], schoolYear: SchoolYear): string | null {
        ...
        try {
            ...
            const schoolYearConditionsWithCountVariant1 = this.getSchoolYearConditionsWithCountMatchRange(HISTORY_SCHOOL_YEAR_CONDITIONS_VARIANT_1);
            let errorMessageVariant1: string = null;
            for (const schoolYearCondition of schoolYearConditionsWithCountVariant1)
                if (isSchoolYearConditionExceedingMax(schoolYearCondition)) {
                    errorMessageVariant1 = `Für die Jahrgänge '${schoolYearRangeToString(schoolYearCondition.schoolYearRange)}' sind im ausgewählten Fach bereits die maximale Anzahl an UBs geplant (${schoolYearCondition.maxAttendances}x).`;
                    break;
                }
            logTrace("current school year counts variant1", schoolYearConditionsWithCountVariant1.map(c => `${schoolYearRangeToString(c.schoolYearRange)}; count: ${c.attendanceCount} - min: ${c.minAttendances} - max: ${c.maxAttendances}`))
                
            // case: variant1 invalid, try variant2
            if (errorMessageVariant1 !== null) {
                const schoolYearConditionsWithCountVariant2 = this.getSchoolYearConditionsWithCountMatchRange(HISTORY_SCHOOL_YEAR_CONDITIONS_VARIANT_2);
                let errorMessageVariant2: string = null;
                for (const schoolYearCondition of schoolYearConditionsWithCountVariant2)
                    if (isSchoolYearConditionExceedingMax(schoolYearCondition)) {
                        errorMessageVariant2 = `Für die Jahrgänge '${schoolYearRangeToString(schoolYearCondition.schoolYearRange)}' sind im ausgewählten Fach bereits die maximale Anzahl an UBs geplant (${schoolYearCondition.maxAttendances}x).`;
                        break;
                    }
                logTrace("current school year counts variant2", schoolYearConditionsWithCountVariant2.map(c => `${schoolYearRangeToString(c.schoolYearRange)}; count: ${c.attendanceCount} - min: ${c.minAttendances} - max: ${c.maxAttendances}`))
    ...
    private concatErrorMessages(message1: string, message2: string): string | null {
        if (isStringFalsy(message1) && isStringFalsy(message2))
            return null;
