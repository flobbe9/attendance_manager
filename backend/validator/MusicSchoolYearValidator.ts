...
export class MusicSchoolYearValidator extends AbstractSchoolYearValidator {
    ...
    public validate(schoolYear: SchoolYear): string | null {
        ...
        if (
            (errorMessage = this.validateNonContextConditions(
                MUSIC_SCHOOL_YEAR_CONDITIONS,
                schoolYear
            )) !== null
        )
        ...
        if (
            (errorMessage = this.validateContextConditions(
                MUSIC_SCHOOL_YEAR_TOPIC_CONDITIONS,
                schoolYear
            )) !== null
        )
    ...
    public validateFuture(schoolYear: SchoolYear): string | null {
        ...
        const savedAttendanceFilterOptions: SchoolYearConditionOptions = {
            includeCurrentAttendanceEntity: this.attendanceService.isSelectInputFilledOut(
        ...
        const currentlyUnsatisfiedTopicConditions = sortSchoolYearConditionsByRangeSize(
            destructSchoolYearConditions(
                this.getCurrentlyUnsatisfiedLessonTopicConditions(
                    MUSIC_SCHOOL_YEAR_TOPIC_CONDITIONS,
                    savedAttendanceFilterOptions
                )
            )
        );
        ...
        try {
            ...
            currentlyUnsatisfiedTopicConditions.forEach((unsatisfiedTopicCondition) => {
                ...
                schoolYearConditionsWithCount.forEach((schoolYearConditionWithCount) => {
            ...
            logTrace(
                ...
                schoolYearConditionsWithCount.map(
                    (c) => `${schoolYearRangeToString(c.schoolYearRange)}; ${c.attendanceCount}`
                )
            );
        } catch (e) {
            return e.message ?? "";
