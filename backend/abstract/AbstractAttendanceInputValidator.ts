 ...
 * @since 0.1.0
 */
export abstract class AbstractAttendanceInputValidator<
    InputValueType extends ValueOf<AttendanceEntity>
> {
    ...
    public getSavedAttendancesWithoutCurrent(): AttendanceEntity[] {
        ...
        const savedCurrentAttendanceIndex = this.savedAttendances.findIndex(
            (savedAttendance) => this.currentAttendance.id === savedAttendance.id
        );
    ...
    public getSavedAttendancesWithCurrent(): AttendanceEntity[] {
        if (!this.currentAttendance || !this.savedAttendances) return this.savedAttendances;
     ...
     */
    protected getSavedAttendancesBySchoolSubject(
        schoolSubject: SchoolSubject_Key,
        includeCurrent: boolean | null = false
    ): AttendanceEntity[] {
        if (!schoolSubject) return [];

        return this.getSavedAttendancesWithOrWithoutCurrent(includeCurrent).filter(
            (attendanceEntity) => attendanceEntity.schoolSubject === schoolSubject
        );
    }
     ...
     * @returns
     */
    protected getSavedAttendancesWithOrWithoutCurrent(
        includeCurrent: boolean | null
    ): AttendanceEntity[] {
        if (includeCurrent === null) return this.savedAttendances;

        return includeCurrent
            ? this.getSavedAttendancesWithCurrent()
     ...
     * @throws if falsy params
     */
    public abstract validateContextConditions(
        constantConditions: any,
        inputValue: InputValueType
    ): string | null;
